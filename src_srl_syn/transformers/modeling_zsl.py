...
PRETRAINED_MODEL_ARCHIVE_MAP = {
    'bert-base-uncased': "/home/zhangsl/.pytorch_pretrained_bert/bert-base-uncased.tar.gz",
    'bert-large-uncased': "/home/zhangsl/.pytorch_pretrained_bert/bert-large-uncased.tar.gz",
    # 'bert-base-cased': "https://s3.amazonaws.com/models.huggingface.co/bert/bert-base-cased.tar.gz",
    'bert-large-cased': "/home/zhangzs/bert-large-cased/bert-large-cased.tar.gz",
    'bert-base-cased': "/home/zhangzs/bert-large-cased/bert-base-cased.tar.gz",
    # 'bert-base-uncased': "/home/zhangzs/mrc/FlowQA/models/bert-base-uncased.tar.gz",
    # 'bert-large-uncased': "/home/wuyw/Bert_Attempt/bert_models/bert-large-uncased/bert-large-uncased.tar.gz",
    # 'bert-base-cased': "/home/wuyw/Bert_Attempt/bert_models/bert-base-cased/bert-base-cased.tar.gz",
    # 'bert-large-cased': "/home/wuyw/Bert_Attempt/bert_models/bert-large-cased/bert-large-cased.tar.gz",
...
class BertForSequenceClassificationTagKD(BertPreTrainedModel):
    def __init__(self, config, num_labels=2, tag_config=None):
        super(BertForSequenceClassificationTagKD, self).__init__(config)
    ...
    def forward(self, input_ids, token_type_ids=None, attention_mask=None, start_end_idx=None, input_tag_ids=None, teacher_probs=None, labels=None):
        ...
        if labels is not None:
            ...
            if teacher_probs is not None:
                logits_logsoftmax = F.log_softmax(logits, dim=-1)
                teacher_probs = F.softmax(teacher_probs, dim=-1)
                teacher_loss = - torch.mean(torch.sum(teacher_probs * logits_logsoftmax, 1))
                loss = loss + self.Lambda * teacher_loss
            return loss
        else:
            return logits
...
def seperate_seq(sequence_output, doc_len, ques_len):
    ...
    for i in range(doc_len.size(0)):
        doc_seq_output[i, ques_len[i]:(doc_len[i] + ques_len[i])] = sequence_output[i, ques_len[i]:(doc_len[i] + ques_len[i])]
        ques_seq_output[i, :ques_len[i]] = sequence_output[i, :ques_len[i]]

    return doc_seq_output, ques_seq_output
...
class MatchNet(nn.Module):
    def __init__(self, config, tag_config=None):
        ...
        self.rank_module = nn.Linear(hidden_size * 2, 1)
...
class BertForSequenceClassificationTagMatch(BertPreTrainedModel):
    def __init__(self, config, num_labels=2, tag_config=None):
        super(BertForSequenceClassificationTagMatch, self).__init__(config)
        self.num_labels = num_labels
        self.bert = BertModel(config)
        self.filter_size = 3
        self.cnn = CNN_conv1d(config, filter_size=self.filter_size)
        ...
        if tag_config is not None:
            ...
            hidden_size = config.hidden_size
        use_tag = True
        self.match = MatchNet(config, tag_config)
        if use_tag:
            self.pool = nn.Linear(2*(config.hidden_size + tag_config.hidden_size), config.hidden_size + tag_config.hidden_size)
            self.classifier = nn.Linear(config.hidden_size + tag_config.hidden_size, num_labels)
        else:
            self.pool = nn.Linear(config.hidden_size, config.hidden_size)
            self.classifier = nn.Linear(config.hidden_size + tag_config.hidden_size, num_labels)
    ...
    def forward(self, input_ids, token_type_ids=None, attention_mask=None, start_end_idx=None, input_tag_ids=None, que_len=None, doc_len=None, labels=None):
        sequence_output, _ = self.bert(input_ids, token_type_ids, attention_mask, output_all_encoded_layers=False)
        is_debug = False
        batch_size, sub_seq_len, dim = sequence_output.size()
        # sequence_output = sequence_output.unsqueeze(1)
        ...
        use_match = True
        if use_match:
            doc_seq_output, ques_seq_output = seperate_seq(sequence_output, doc_len, que_len)
            sequence_output = self.match([doc_seq_output, ques_seq_output, que_len])
...
class BertForSequenceScoreTagMatch(BertPreTrainedModel):
    def __init__(self, config, tag_config=None):
        super(BertForSequenceScoreTagMatch, self).__init__(config)
        self.bert = BertModel(config)
        self.filter_size = 3
        self.cnn = CNN_conv1d(config, filter_size=self.filter_size)
...
def seperate_seq(sequence_output, doc_len, option_len):
    doc_seq_output = sequence_output.new(sequence_output.size()).zero_()
    ...
    for i in range(len(doc_len)):
        ...
        option_seq_output[i, :option_len[i]] = sequence_output[i, doc_len[i] + 1:doc_len[i]  + option_len[
                                                   i] + 1]
    return doc_seq_output, option_seq_output

class BertForMultipleChoiceWithTwo(BertPreTrainedModel):

    def __init__(self, config, num_labels):
        super(BertForMultipleChoiceWithTwo, self).__init__(config)
        self.num_labels = num_labels
        self.bert = BertModel(config)
        self.dropout = nn.Dropout(config.hidden_dropout_prob)
        self.classifier = nn.Linear(config.hidden_size, num_labels)
        ...
        #self.match_linear = nn.Linear(2 * config.hidden_size, config.hidden_size)
        self.match = MatchNet(config)
        self.smatch = SingleMatchNet(config)
        self.dualmatch = DualMatchNet(config)
        self.pooler = BertPooler(config)
        self.fuse = FuseNet(config)
        self.apply(self.init_bert_weights)

    def forward(self, input_ids=None, token_type_ids=None, attention_mask=None, doc_len=None, option_len=None, labels=None):
        flat_input_ids = input_ids.view(-1, input_ids.size(-1))

        doc_len = doc_len.view(-1, doc_len.size(-1)).squeeze(0)
        option_len = option_len.view(-1, option_len.size(-1)).squeeze(0)

        # print("doc_len:", doc_len.size(), doc_len)
        ...
        doc_seq_output, option_seq_output = seperate_seq(sequence_output, doc_len, option_len)
        ...
        # output_pool = torch.cat([pa_output_pool, pq_o
        output_pool = self.fuse([pa_output_pool, ap_output_pool])
        ...
        if labels is not None:
            loss_fct = CrossEntropyLoss()
            loss = loss_fct(match_logits.view(-1, self.num_labels), labels)
            return loss
        else:
            return match_logits
        ...
        #     return lam*loss + (1-lam)*match_loss
        # else:
        #     return lam*reshaped_logits + (1-lam)*match_reshaped_logits
        #
        # # if labels is not None:
        #     loss_fct = CrossEntropyLoss()
        #     loss = loss_fct(logits.view(-1, self.num_labels), labels.view(-1))
