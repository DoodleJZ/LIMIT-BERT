...
def decode(int sentence_len, np.ndarray[DTYPE_t, ndim=3] label_scores_chart, np.ndarray[DTYPE_t, ndim=2] type_scores_chart):
    ...
    return score, included_i.astype(int), included_j.astype(int), included_label.astype(int), included_father.astype(int)
